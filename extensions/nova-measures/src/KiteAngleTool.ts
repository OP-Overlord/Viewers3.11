...
interface KiteAngleData {
  handles: {
    segment1: [Types.Point3, Types.Point3]; // Primer segmento (2 puntos)
    segment2: [Types.Point3, Types.Point3]; // Segundo segmento (2 puntos)
    textBox: {
      hasMoved: boolean;
  ...
  cachedStats: {
    angle?: number; // Ángulo de Kite en grados
    intersection?: Types.Point3; // Punto de intersección (si existe)
  };
}

interface KiteAngleAnnotation {
...
export default class KiteAngleTool extends AnnotationTool {
  public static toolName = 'KiteAngle';

  private editData: {
    annotation: KiteAngleAnnotation;
  ...
  addNewAnnotation = (evt: CstTypes.InteractionEventType): KiteAngleAnnotation | undefined => {
    ...
    const newAnnotation: KiteAngleAnnotation = {
      highlighted: true,
      invalidated: true,
      metadata: {
  ...
  private _updateTextBoxPosition(annotation: KiteAngleAnnotation) {
    const data = annotation.data as KiteAngleData;
  ...
  private _calculateStats(annotation: KiteAngleAnnotation) {
    const data = annotation.data as KiteAngleData;
    const [p1s1, p2s1] = data.handles.segment1;
    const [p1s2, p2s2] = data.handles.segment2;
  ...
  renderAnnotation = (enabledElement: any, svgDrawingHelper: any): boolean => {
    ...
    for (const ann of annotations) {
      ...
      if (!isNewAnnotation && d.handles.textBox?.worldPosition && angle !== undefined) {
        ...
        const textLines = [`Ángulo de Kite: ${angle.toFixed(1)}°`];
        const lineHeight = 14;
        const svgns = 'http://www.w3.org/2000/svg';
        const svgLayer = svgDrawingHelper.svgLayerElement;
